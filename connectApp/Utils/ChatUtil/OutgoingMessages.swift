...
class OutgoingMessage {
    ...
    func sendMessage(chatRoomID: String, messageDictionary: NSMutableDictionary, memberIds: [String], membersToPush: [String]) {
        ...
        for memberId in memberIds {
            collectionReference(.Message).document(memberId).collection(chatRoomID).document(messageId).setData(messageDictionary as! [String : Any])
        }
    ...
    class func deleteMessage(withId: String, chatRoomId: String) {
        collectionReference(.Message).document(FUser.currentID()).collection(chatRoomId).document(withId).delete()
    }
    ...
    class func updateMessage(withId: String, chatRoomId: String, memberIds: [String]) {
        ...
        for userId in memberIds {
            ...
            collectionReference(.Message).document(userId).collection(chatRoomId).document(withId).getDocument { (snapshot, error) in
                
                guard let snapshot = snapshot  else { return }
                
                if snapshot.exists {
                    
                    collectionReference(.Message).document(userId).collection(chatRoomId).document(withId).updateData(values)
                }
            }
        }
    }
