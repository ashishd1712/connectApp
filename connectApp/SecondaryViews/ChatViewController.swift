...
class ChatViewController: JSQMessagesViewController, UIImagePickerControllerDelegate, UINavigationControllerDelegate, IQAudioRecorderViewControllerDelegate {
    ...
    override func viewDidLoad() {
        ...
        //        //navigationItem.largeTitleDisplayMode = .never
        //        self.navigationItem.leftBarButtonItems = [UIBarButtonItem(image: UIImage(systemName: "arrow.left"), style: .plain, target: self, action: #selector(self.backAction))]
        ...
        loadMessages()
        print("FUser.currentID = \(FUser.currentID())")
    ...
    override func collectionView(_ collectionView: JSQMessagesCollectionView!, didTapMessageBubbleAt indexPath: IndexPath!) {
        ...
        case kPICTURE:
            ...
            let photos = IDMPhoto.photos(withImages: [mediaItem.image] as [UIImage])
        ...
        case kLOCATION:
            ...
            let mapView = UIStoryboard(name: "Main", bundle: nil).instantiateViewController(withIdentifier: StoryboardID.mapViewController) as! MapViewController
...
//    }
    ...
    func sendMessage(text: String?, date: Date, picture: UIImage?, location: String?, video: NSURL?, audio: String?) {
        ...
        if let text = text {
            ...
            outgoingMessage = OutgoingMessage(message: ecryptedText, senderId: currentUser.objectID, senderName: currentUser.fullName, date: date, status: kDELIVERED, type: kTEXT)
        }
        ...
        if let pic = picture {
            ...
            uploadImage(image: pic, chatRoomId: chatRoomId, view: self.navigationController!.view) { (imageLink) in
                ...
                if imageLink != nil {
                    ...
                    outgoingMessage = OutgoingMessage(message: ecryptedText, pictureLink: imageLink!, senderId: currentUser.objectID, senderName: currentUser.fullName, date: date, status: kDELIVERED, type: kPICTURE)
        ...
        if let video = video {
            ...
            uploadVideo(video: videoData!, chatRoomId: chatRoomId, view: self.navigationController!.view) { (videoLink) in
                ...
                if videoLink != nil {
                    ...
                    outgoingMessage = OutgoingMessage(message: ecryptedText, video: videoLink!, thumbNail: dataThumbnail! as NSData, senderId: currentUser.objectID, senderName: currentUser.fullName, date: date, status: kDELIVERED, type: kVIDEO)
        ...
        if let audioPath = audio {
            ...
            uploadAudio(autioPath: audioPath, chatRoomId: chatRoomId, view: (self.navigationController?.view)!) { (audioLink) in
                ...
                if audioLink != nil {
                    ...
                    outgoingMessage = OutgoingMessage(message: ecryptedText, audio: audioLink!, senderId: currentUser.objectID, senderName: currentUser.fullName, date: date, status: kDELIVERED, type: kAUDIO)
        ...
        if location != nil {
            ...
            outgoingMessage = OutgoingMessage(message: ecryptedText, latitude: lat, longitude: long, senderId: currentUser.objectID, senderName: currentUser.fullName, date: date, status: kDELIVERED, type: kLOCATION)
        }
    ...
    func loadMessages() {
        ...
        updatedChatListener = collectionReference(.Message).document(FUser.currentID()).collection(chatRoomId).addSnapshotListener({ (snapshot, error) in
        ...
        collectionReference(.Message).document(FUser.currentID()).collection(chatRoomId).order(by: kDATE, descending: true).limit(to: 11).getDocuments { (snapshot, error) in
            ...
            let sorted = ((dictionaryFromSnapshot(snapshots: snapshot.documents)) as NSArray).sortedArray(using: [NSSortDescriptor(key: kDATE, ascending: true)]) as! [NSDictionary]
    ...
    func listenForNewChats() {
        ...
        newChatListener = collectionReference(.Message).document(FUser.currentID()).collection(chatRoomId).whereField(kDATE, isGreaterThan: lastMessageDate).addSnapshotListener({ (snapshot, error) in
    ...
    func getOldMessagesInBackground() {
        ...
        if loadedMessages.count > 10 {
            ...
            collectionReference(.Message).document(FUser.currentID()).collection(chatRoomId).whereField(kDATE, isLessThan: firstMessageDate).getDocuments { (snapshot, error) in
...
//        self.navigationController?.pushViewController(profileVC, animated: true)
    ...
    func createTypingObserver() {
        ...
        typingListener = collectionReference(.Typing).document(chatRoomId).addSnapshotListener({ (snapshot, error) in
            ...
            if snapshot.exists {
                ...
                collectionReference(.Typing).document(self.chatRoomId).setData([FUser.currentID() : false])
            }
    ...
    func typingCounterSave(typing: Bool) {
        ...
        collectionReference(.Typing).document(chatRoomId).updateData([FUser.currentID() : typing])
    }
    ...
    func setUIForGroupChat() {
        ...
        imageFromData(imageData: (group![kAVATAR] as! String)) { (image) in
    ...
    func setUIForSingleChat() {
        ...
        imageFromData(imageData: withUser.avatar) { (image) in
    ...
    func avatarImageFrom(fUser: FUser) {
        ...
        if fUser.avatar != "" {
            ...
            dataImageFromString(imageString: fUser.avatar) { (imageData) in
    ...
    func createJSQAvatars(avatarDictionary: NSMutableDictionary?) {
        ...
        let defaultAvatar = JSQMessagesAvatarImageFactory.avatarImage(with: UIImage(systemName: "person"), diameter: 70)
    ...
    func haveAccessToUserLocation() -> Bool {
        if appDelegate.locationManager != nil {
            ...
            ProgressHUD.error("Please give access tp loacation in Settings.")
            return false
        }
    }
    ...
    func getCurrentGroup(withId: String) {
        ...
        collectionReference(.Group).document(withId).getDocument { (snapshot, error) in
